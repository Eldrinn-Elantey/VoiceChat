...
public class RenderPlayerVoiceIcon extends Gui {
    ...
    public void render(RenderWorldLastEvent event) {
        if (!VoiceChatClient.getSoundManager().currentStreams.isEmpty() && voiceChat.getSettings()
            ...
            for (int i = 0; i < MathUtility.clamp(
                VoiceChatClient.getSoundManager().currentStreams.size(),
                0,
                voiceChat.getSettings()
                    .getMaximumRenderableVoiceIcons()); i++) {
    ...
    public void translateEntity(Entity entity, float tick) {
        glTranslated(
            entity.prevPosX + (entity.posX - entity.prevPosX) * tick,
            entity.prevPosY + (entity.posY - entity.prevPosY) * tick,
            entity.prevPosZ + (entity.posZ - entity.prevPosZ) * tick);
    }

    public void translateWorld(Minecraft mc, float tick) {
        glTranslated(
            -(mc.thePlayer.prevPosX + (mc.thePlayer.posX - mc.thePlayer.prevPosX) * tick),
            -(mc.thePlayer.prevPosY + (mc.thePlayer.posY - mc.thePlayer.prevPosY) * tick),
            -(mc.thePlayer.prevPosZ + (mc.thePlayer.posZ - mc.thePlayer.prevPosZ) * tick));
    }
}
