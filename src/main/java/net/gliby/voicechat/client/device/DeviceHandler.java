...
public class DeviceHandler {
    ...
    public List<Device> loadDevices() {
        ...
        for (final Mixer.Info info : mixers) {
            ...
            try {
                final javax.sound.sampled.DataLine.Info tdlLineInfo = new DataLine.Info(
                    TargetDataLine.class,
                    ClientStreamManager.getUniversalAudioFormat());
                final TargetDataLine tdl = (TargetDataLine) mixer.getLine(tdlLineInfo);
                if (info != null) devices.add(new Device(tdl, info));
            } catch (final LineUnavailableException e) {} catch (final IllegalArgumentException e) {}
        }
        return devices;
    }
}
