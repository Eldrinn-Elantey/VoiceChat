...
public class Recorder implements Runnable {
    ...
    private byte[] boostVolume(byte[] data) {
        ...
        final ByteBuffer newBuf = ByteBuffer.allocate(data.length)
            .order(ByteOrder.LITTLE_ENDIAN);
        int sample;
        while (buf.hasRemaining()) {
            sample = buf.getShort() & USHORT_MASK;
            sample *= (int) (voiceChat.getSettings()
                .getInputBoost() * 5) + 1;
    ...
    public void run() {
        ...
        final TargetDataLine recordingLine = voiceChat.getSettings()
            .getInputDevice()
            .getLine();
        if (recordingLine == null) {
            VoiceChat.getLogger()
                .fatal(
                    "Attempted to record input device, but failed! Java Sound System hasn't found any microphones, check your input devices and restart Minecraft.");
            return;
        }
        ...
        encoder.init(
            0,
            (int) MathUtility.clamp(
                MathUtility.clamp(
                    (int) (voiceChat.getSettings()
                        .getEncodingQuality() * 10.0f),
                    1,
                    9),
        ...
        buffer = new byte[0];
        byte pieceSize = 0;
        while (recording && voiceChat.getClientNetwork()
            .isConnected()) {
            ...
            write(encoded_data);
            if (buffer.length >= voiceChat.getSettings()
                .getBufferSize()) {
                voiceChat.getClientNetwork()
                    .sendSamples(pieceSize, buffer, false);
                buffer = new byte[0];
            }
        }
        if (buffer.length > 0) {
    ...
    private boolean startLine(TargetDataLine recordingLine) {
        try {
            ...
            e.printStackTrace();
            VoiceChat.getLogger()
                .fatal("Failed to open recording line! " + recordingLine.getFormat());
            return false;
        }
        return true;
    }
