...
public class MicrophoneTester implements Runnable {
    ...
    private byte[] boostVolume(byte[] data) {
        ...
        while (buf.hasRemaining()) {
            ...
            sample *= 1 + (int) (voiceChat.getSettings()
                .getInputBoost() * 5);
    ...
    public void run() {
        ...
        line = voiceChat.getSettings()
            .getInputDevice()
            .getLine();
        if (line == null) {
            VoiceChatClient.getLogger()
                .fatal("No line in found, cannot test input device.");
            return;
        }
        final DataLine.Info sourceInfo = new DataLine.Info(
            SourceDataLine.class,
            ClientStreamManager.universalAudioFormat);
        try {
