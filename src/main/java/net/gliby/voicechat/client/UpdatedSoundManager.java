...
public class UpdatedSoundManager {
    ...
    public UpdatedSoundManager(VoiceChatClient voiceChatClient) {
        for (final ModContainer mod : Loader.instance()
            .getModList()) {
            if (mod.getModId()
                .equals("soundfilters")) {
                VoiceChat.getLogger()
                    .info("Found Sound Filters mod, won't replace OpenAL library.");
                return;
            }
        }
        ...
        try {
            ...
            VoiceChat.getLogger()
                .info("Failed to replaced sound libraries, you won't be hearing any voice chat.");
            e.printStackTrace();
        }
        VoiceChat.getLogger()
            .info("Successfully replaced sound libraries.");
    }
