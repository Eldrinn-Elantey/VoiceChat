...
public class GuiScreenVoiceChatOptionsAdvanced extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 1:
                ...
                break;
            case 5:
                int mode = voiceChat.getSettings()
                    .getEncodingMode();
                if (mode < 2) mode++;
                else mode = 0;
                voiceChat.getSettings()
                    .setEncodingMode(mode);
                encodingMode.displayString = I18n.format("menu.encodingMode") + ": "
                    + voiceChat.getSettings()
                        .getEncodingModeString();
                break;
            case 6:
                voiceChat.getSettings()
                    .setPerceptualEnchantment(
                        !voiceChat.getSettings()
                            .isPerceptualEnchantmentAllowed());
                enhancedDecoding.displayString = I18n.format("menu.enhancedDecoding") + ": "
                    + (voiceChat.getSettings()
                        .isPerceptualEnchantmentAllowed() ? I18n.format("options.on") : I18n.format("options.off"));
                break;
            case 7:
                voiceChat.getSettings()
                    .setSnooperAllowed(false);
                serverConnection.displayString = I18n.format("menu.allowSnooper") + ": "
                    + (voiceChat.getSettings()
                        .isSnooperAllowed() ? I18n.format("options.on") : I18n.format("options.off"));
                break;
            case 8:
                voiceChat.getSettings()
                    .setVolumeControl(
                        !voiceChat.getSettings()
                            .isVolumeControlled());
                volumeControlButton.displayString = I18n.format("menu.volumeControl") + ": "
                    + (voiceChat.getSettings()
                        .isVolumeControlled() ? I18n.format("options.on") : I18n.format("options.off"));
                voiceChat.getSoundManager()
                    .volumeControlStop();
                break;
        }
    }
    ...
    public void drawScreen(int x, int y, float time) {
        ...
        glPopMatrix();
        if ((int) (voiceChat.getSettings()
            .getEncodingQuality() * 10) <= 2) {
    ...
    public void initGui() {
        ...
        qualitySlider = new GuiBoostSlider(
            4,
            this.width / 2 + 2,
            74,
            "",
            I18n.format("menu.encodingQuality") + ": "
                + (voiceChat.getSettings()
                    .getEncodingQuality() == 0 ? "0"
                        : String.valueOf(
                            (int) (voiceChat.getSettings()
                                .getEncodingQuality() * 10))),
            0);
        qualitySlider.sliderValue = voiceChat.getSettings()
            .getEncodingQuality();
        encodingMode = new GuiButton(
            5,
            (this.width / 2) - 152,
            98,
            150,
            20,
            I18n.format("menu.encodingMode") + ": "
                + voiceChat.getSettings()
                    .getEncodingModeString());
        buttonList
            .add(
                enhancedDecoding = new GuiButton(
                    6,
                    (this.width / 2) - 152,
                    50,
                    150,
                    20,
                    I18n.format("menu.enhancedDecoding") + ": "
                        + (voiceChat.getSettings()
                            .isPerceptualEnchantmentAllowed() ? I18n.format("options.on")
                                : I18n.format("options.off"))));
        buttonList.add(
            serverConnection = new GuiButton(
                7,
                (this.width / 2) + 2,
                50,
                150,
                20,
                I18n.format("menu.allowSnooper") + ": "
                    + (voiceChat.getSettings()
                        .isSnooperAllowed() ? I18n.format("options.on") : I18n.format("options.off"))));
        buttonList.add(
            volumeControlButton = new GuiButton(
                8,
                (this.width / 2) - 152,
                74,
                150,
                20,
                I18n.format("menu.volumeControl") + ": "
                    + (voiceChat.getSettings()
                        .isVolumeControlled() ? I18n.format("options.on") : I18n.format("options.off"))));
    ...
    public void resetAdvancedOptions() {
        ...
        voiceChat.getSettings()
            .setEncodingQuality(qualitySlider.sliderValue);
        qualitySlider.idValue = I18n.format("menu.encodingQuality") + ": "
            + (voiceChat.getSettings()
                .getEncodingQuality() == 0 ? "0"
                    : String.valueOf(
                        (int) (voiceChat.getSettings()
    ...
    public void updateScreen() {
        voiceChat.getSettings()
            .setEncodingQuality(qualitySlider.sliderValue);
        qualitySlider.setDisplayString(
            I18n.format("menu.encodingQuality") + ": "
                + (voiceChat.getSettings()
                    .getEncodingQuality() == 0 ? "0"
                        : String.valueOf(
