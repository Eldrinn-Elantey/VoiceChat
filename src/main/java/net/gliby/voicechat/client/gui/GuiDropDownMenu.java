...
public class GuiDropDownMenu extends GuiButton {
    ...
    public void drawButton(Minecraft par1Minecraft, int x, int y) {
        if (this.visible) {
            ...
            drawRect(
                this.xPosition - 1,
                this.yPosition + this.prevHeight,
                this.xPosition + this.width + 1,
                this.yPosition + this.prevHeight + 1,
                -6250336);
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
            int u = 256 - 14;
            ...
            this.drawCenteredString(
                fontrenderer,
                this.displayString.substring(0, Math.min(displayString.length(), 22)),
                this.xPosition + this.width / 2,
                this.yPosition + (this.prevHeight - 8) / 2,
                l);
            GL11.glPushMatrix();
            if (dropDownMenu && array.length != 0) {
                for (int i = 0; i < amountOfItems; i++) {
                    ...
                    this.drawCenteredString(
                        ...
                        s,
                        this.xPosition + this.width / 2,
                        this.yPosition + (this.prevHeight * (i + 1)) + 15 / 2,
                        mouseOn[i] ? 16777120 : 14737632);
                }
            }
            GL11.glPopMatrix();
            GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
