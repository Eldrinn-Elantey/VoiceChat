...
public class GuiScreenLocalMute extends GuiScreen {
    ...
    class List extends GuiSlot {
        ...
        public List() {
            super(
                GuiScreenLocalMute.this.mc,
                GuiScreenLocalMute.this.width,
                GuiScreenLocalMute.this.height,
                32,
                GuiScreenLocalMute.this.height - 65 + 4,
                18);
        ...
        @Override
        protected void drawSlot(int valueId, int par1, int par2, int par3, Tessellator tessellator, int par4,
            int par5) {
            GuiScreenLocalMute.this.drawCenteredString(
                GuiScreenLocalMute.this.fontRendererObj,
                VoiceChatClient.getSoundManager().playerMutedData
                    .get(VoiceChatClient.getSoundManager().playersMuted.get(valueId)),
                this.width / 2,
                par2 + 1,
                16777215);
            GuiScreenLocalMute.this.drawCenteredString(
    ...
    private void actionPerformed(int button) {
        switch (button) {
            case 0:
                ...
                final EntityPlayer entityPlayer = mc.theWorld.getPlayerEntityByName(
                    playerTextField.getText()
                        .trim()
                        .replaceAll(" ", ""));
                if (entityPlayer != null) {
                    if (!entityPlayer.isClientWorld()
                        && !VoiceChatClient.getSoundManager().playersMuted.contains(entityPlayer.getEntityId())) {
                        VoiceChatClient.getSoundManager().playersMuted.add(entityPlayer.getEntityId());
                        VoiceChatClient.getSoundManager();
                        ClientStreamManager.playerMutedData
                            .put(entityPlayer.getEntityId(), entityPlayer.getCommandSenderName());
                    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        if (playerNotFound) {
            this.drawCenteredString(
                ...
                "\247c" + I18n.format("commands.generic.player.notFound"),
                this.width / 2,
                height - 59,
                -1);
        }
    ...
    public void initGui() {
        ...
        playerTextField = new GuiTextField(
            this.fontRendererObj,
            width / 2 - 100,
            this.height - 57 - heightOffset,
            130,
            20);
        playerTextField.setFocused(true);
        this.buttonList.add(
            this.doneButton = new GuiOptionButton(
                0,
                this.width / 2 + 32,
                this.height - 57 - heightOffset,
                98,
                20,
                I18n.format("menu.add")));
        this.buttonList.add(
            this.doneButton = new GuiOptionButton(
                1,
                this.width / 2 - 75,
                this.height - 32 - heightOffset,
    ...
    private void shuffleCompleition() {
        if (autoCompletionNames.iterator()
            .hasNext()) {
            final String name = autoCompletionNames.iterator()
                .next();
