...
public class GuiScreenVoiceChatOptions extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 898:
                ...
                break;
            case 0:
                if (button instanceof GuiDropDownMenu && !voiceChat.getSettings()
                    .getDeviceHandler()
                    .isEmpty()) ((GuiDropDownMenu) button).dropDownMenu = !((GuiDropDownMenu) button).dropDownMenu;
                break;
            case 1:
                voiceChat.getSettings()
                    ...
                    .save();
                mc.displayGuiScreen(
                    new GuiScreenDonate(voiceChat.getModInfo(), VoiceChatClient.getModMetadata(), this));
                break;
            case 2:
                if (!tester.recording) tester.start();
                else tester.stop();
                button.displayString = tester.recording ? I18n.format("menu.microphoneStopTest")
                    : I18n.format("menu.microphoneTest");
                break;
            case 3:
                voiceChat.getSettings()
                    .getConfiguration()
                ...
                if (!dropDown.dropDownMenu) {
                    ...
                    voiceChat.getSettings()
                        .setSpeakMode(
                            voiceChat.getSettings()
                                .getSpeakMode() == 0 ? 1 : 0);
                    microphoneMode.displayString = I18n.format("menu.speakMode") + ": "
                        + (voiceChat.getSettings()
                            .getSpeakMode() == 0 ? I18n.format("menu.speakModePushToTalk")
                                : I18n.format("menu.speakModeToggleToTalk"));
                } else if (voiceChat.getSettings()
                    .getDeviceHandler()
                    .isEmpty()) {
    ...
    public void drawScreen(int x, int y, float tick) {
        ...
        for (int i = 0; i < warningMessages.size(); i++) {
            final int warnY = i * fontRendererObj.FONT_HEIGHT + height / 2
                + 66
                - ((fontRendererObj.FONT_HEIGHT * warningMessages.size()) / 2);
    ...
    @Override
    public void initGui() {
        final String[] array = new String[voiceChat.getSettings()
            .getDeviceHandler()
            .getDevices()
            .size()];
        for (int i = 0; i < voiceChat.getSettings()
            .getDeviceHandler()
            .getDevices()
            .size(); i++) {
            array[i] = voiceChat.getSettings()
                .getDeviceHandler()
                .getDevices()
                .get(i)
                .getName();
        ...
        dropDown = new GuiDropDownMenu(
            ...
            20,
            voiceChat.getSettings()
                .getInputDevice() != null ? voiceChat.getSettings()
                    .getInputDevice()
                    .getName() : "None",
            array);
        microphoneMode = new GuiButton(
            5,
            this.width / 2 - 152,
            (height / 2 + 25) - heightOffset,
            150,
            20,
            I18n.format("menu.speakMode") + ": "
                + (voiceChat.getSettings()
                    .getSpeakMode() == 0 ? I18n.format("menu.speakModePushToTalk")
                        : I18n.format("menu.speakModeToggleToTalk")));
        UIPosition = new GuiButton(
            4,
            this.width / 2 + 2,
            (height / 2 + 25) - heightOffset,
            150,
            20,
            I18n.format("menu.uiOptions"));
        voiceVolume = new GuiBoostSlider(
            910,
            this.width / 2 + 2,
            (height / 2 - 25) - heightOffset,
            "",
            I18n.format("menu.worldVolume") + ": "
                + (voiceChat.getSettings()
                    .getWorldVolume() == 0 ? I18n.format("options.off")
                        : String.valueOf(
                            (int) (voiceChat.getSettings()
                                .getWorldVolume() * 100))
                            + "%"),
            0);
        voiceVolume.sliderValue = voiceChat.getSettings()
            .getWorldVolume();
        boostSlider = new GuiBoostSlider(
            900,
            this.width / 2 + 2,
            (height / 2) - heightOffset,
            "",
            I18n.format("menu.boost") + ": "
                + ((int) (voiceChat.getSettings()
                    .getInputBoost() * 5f) <= 0 ? I18n.format("options.off")
                        : "" + (int) (voiceChat.getSettings()
                            .getInputBoost() * 5) + "db"),
            0);
        boostSlider.sliderValue = voiceChat.getSettings()
            .getInputBoost();
        advancedOptions = new GuiCustomButton(
            899,
            this.width / 2 + 2,
            (height / 2 + 49) - heightOffset,
            150,
            20,
            I18n.format("menu.advancedOptions"));
        buttonList
            .add(new GuiButton(1, (this.width / 2) - 151, height - 34, 75, 20, I18n.format("menu.gman.supportGliby")));
        buttonList.add(
            new GuiButton(
                2,
                width / 2 - 152,
                ...
                150,
                20,
                !tester.recording ? I18n.format("menu.microphoneTest") : I18n.format("menu.microphoneStopTest")));
        ...
        if (voiceChat.getSettings()
            ...
            .isEmpty())
            warningMessages
                .add(EnumChatFormatting.DARK_RED + "No input devices found, add input device and restart Minecraft.");

        if (voiceChat.getModInfo()
            .updateNeeded()) {
            warningMessages
                .add(updateMessage = I18n.format("menu.downloadLatest") + "\247b " + voiceChat.getModInfo().updateURL);
            warningMessages.add(EnumChatFormatting.RED + I18n.format("menu.modOutdated"));
        }

        if (mc.isSingleplayer() && !mc.getIntegratedServer()
            .getPublic()) warningMessages.add(EnumChatFormatting.RED + I18n.format("menu.warningSingleplayer"));

        if (!voiceChat.getClientNetwork()
            .isConnected() && !mc.isSingleplayer()) {
            ...
            if (voiceChat.getModInfo()
                ...
                for (int i = 0; i < warningMessages.size(); i++) {
                    ...
                    if (s.equals(updateMessage)) {
                        ...
                        if (inBounds(x, y, width / 2 - (length / 2), warnY, length, fontRendererObj.FONT_HEIGHT))
                            this.openURL(voiceChat.modInfo.updateURL);
                    }
                }
            }
            if (dropDown.getMouseOverInteger() != -1 && dropDown.dropDownMenu
                && !voiceChat.getSettings()
                    .getDeviceHandler()
                    .isEmpty()) {
                final Device device = voiceChat.getSettings()
                    .getDeviceHandler()
                    .getDevices()
                    .get(dropDown.getMouseOverInteger());
    ...
    private void openURL(String par1URI) {
        try {
            ...
            final Object object = oclass.getMethod("getDesktop", new Class[0])
                .invoke((Object) null, new Object[0]);
            oclass.getMethod("browse", new Class[] { URI.class })
                .invoke(object, new Object[] { new URI(par1URI) });
    ...
    public void updateScreen() {
        voiceChat.getSettings()
            .setWorldVolume(voiceVolume.sliderValue);
        voiceChat.getSettings()
            .setInputBoost(boostSlider.sliderValue);
        voiceVolume.setDisplayString(
            I18n.format("menu.worldVolume") + ": "
                + (voiceChat.getSettings()
                    .getWorldVolume() == 0 ? I18n.format("options.off")
                        : +(int) (voiceChat.getSettings()
                            .getWorldVolume() * 100) + "%"));
        boostSlider.setDisplayString(
            I18n.format("menu.boost") + ": "
                + ((int) (voiceChat.getSettings()
                    .getInputBoost() * 5f) <= 0 ? I18n.format("options.off")
                        : (int) (voiceChat.getSettings()
