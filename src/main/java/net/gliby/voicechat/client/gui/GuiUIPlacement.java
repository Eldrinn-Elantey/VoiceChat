...
public class GuiUIPlacement extends GuiScreen {
    ...
    public void drawScreen(int x, int y, float tick) {
        ...
        if (lastSelected != null) {
            ...
            final boolean rightSide = inBounds(
                lastSelected.x + lastSelected.width + 151,
                lastSelected.y + 42,
                width,
                0,
                width,
                height * 2);
            final boolean topSide = inBounds(
                lastSelected.x + lastSelected.width - 75,
                lastSelected.y,
                -width,
                -height,
                width * 2,
                height);
            final boolean bottomSide = inBounds(
                lastSelected.x + lastSelected.width,
                lastSelected.y + 66,
                0,
                height,
                width * 2,
            ...
            positionTypeButton.yPosition = (int) (lastSelected.y
                - (bottomSide ? (lastSelected.y + 66) - height : (topSide ? lastSelected.y - 0 : 0)));
            scaleSlider.xPosition = (int) (lastSelected.x + (rightSide ? -154 : lastSelected.width + 2));
            scaleSlider.yPosition = (int) (lastSelected.y + 22
                - (bottomSide ? (lastSelected.y + 66) - height : (topSide ? lastSelected.y - 0 : 0)));
            resetButton.xPosition = (int) (lastSelected.x + (rightSide ? -100 : lastSelected.width + 2));
            resetButton.yPosition = (int) (lastSelected.y + 44
                - (bottomSide ? (lastSelected.y + 66) - height : (topSide ? lastSelected.y - 0 : 0)));
            positionTypeButton.displayString = I18n.format("menu.position") + ": "
                + positionTypes[lastSelected.positionType];
    ...
    private void resize(GuiPlaceableInterface placeable) {
        placeable.update(
            (int) (width * ((placeable.x * 1.0F) / placeable.screenWidth)),
            (int) (height * ((placeable.y * 1.0F) / placeable.screenHeight)),
            width,
            height);
    }

    public void save() {
        final Settings settings = VoiceChat.getProxyInstance()
            .getSettings();
        for (int i = 0; i < placeables.size(); i++) {
            final GuiPlaceableInterface placeable = placeables.get(i);
