...
public class GuiScreenOptionsWizard extends GuiScreen {
    ...
    public void actionPerformed(GuiButton button) {
        if ((button == nextButton || button == previousButton || doneButton == button)
            || (buttonMap.get(button) != null ? buttonMap.get(button) == currentPage : false)) {
    ...
    public void drawScreen(int x, int y, float tick) {
        ...
        if (title != null) drawString(
            mc.fontRenderer,
            EnumChatFormatting.BOLD + title,
            (width / 2) - (mc.fontRenderer.getStringWidth(title) / 2) - 12,
            height / 2 - 80,
            -1);
        if (text != null) {
        ...
        for (int k = 0; k < this.buttonList.size(); ++k) {
            ...
            if ((guibutton == nextButton || guibutton == previousButton || guibutton == doneButton)
                || (buttonMap.get(guibutton) != null ? buttonMap.get(guibutton) == currentPage : false)) {
    ...
    public void initGui() {
        ...
        dropDown = new GuiDropDownMenu(
            ...
            array);
        buttonList.add(
            nextButton = new GuiCustomButton(
                0,
                width / 2 - 90,
                height / 2 + 60,
                180,
                20,
                I18n.format("menu.next") + " ->"));
        buttonList.add(
            previousButton = new GuiCustomButton(
                1,
                width / 2 - 90,
                height / 2,
                180,
                20,
                "<- " + I18n.format("menu.previous")));
        buttonList
            .add(doneButton = new GuiCustomButton(2, width / 2 - 90, height / 2, 180, 20, I18n.format("gui.done")));
        buttonList.add(
            backButton = new GuiCustomButton(3, width / 2 - 90, height / 2 + 18, 180, 20, I18n.format("gui.back")));
        buttonList
            .add(
        ...
        dirty = true;
        textBatch = new String[] { I18n.format("menu.setupWizardPageOne")
            .replaceAll(Pattern.quote("$n"), "\n")
            .replaceAll(Pattern.quote("$a"), voiceChat.keyManager.getKeyName(EnumBinding.OPEN_GUI_OPTIONS)),
            ...
            I18n.format("menu.setupWizardPageFour")
                .replaceAll(Pattern.quote("$n"), "\n")
                .replaceAll(Pattern.quote("$a"), voiceChat.keyManager.getKeyName(EnumBinding.OPEN_GUI_OPTIONS))
                .replaceAll(Pattern.quote("$b"), voiceChat.keyManager.getKeyName(EnumBinding.SPEAK)) };
    }
    ...
    public void mouseClicked(int x, int y, int b) {
        ...
        if (b == 0) {
            for (int l = 0; l < this.buttonList.size(); ++l) {
                ...
                if ((guibutton == nextButton || guibutton == previousButton || doneButton == guibutton)
                    || (buttonMap.get(guibutton) != null ? buttonMap.get(guibutton) == currentPage : false)) {
