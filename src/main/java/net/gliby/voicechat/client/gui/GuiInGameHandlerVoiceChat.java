...
public class GuiInGameHandlerVoiceChat extends Gui {
    ...
    private Vector2f getPosition(int width, int height, UIPosition uiPositionSpeak) {
        return uiPositionSpeak.type == 0 ? new Vector2f(uiPositionSpeak.x * width, uiPositionSpeak.y * height)
            : new Vector2f(uiPositionSpeak.x, uiPositionSpeak.y);
    }
    ...
    public void renderInGameGui(RenderGameOverlayEvent.Post event) {
        if (event.type == ElementType.HOTBAR) {
            ...
            calcDelta();
            if (!VoiceChat.getProxyInstance()
                .isRecorderActive()) if (fade > 0) fade -= 0.01f * delta;
            else fade = 0;
            else if (fade < 1.0f && VoiceChat.getProxyInstance()
            ...
            if (fade != 0 && voiceChat.getSettings()
                ...
                if (positionUI.scale != 0) {
                    ...
                    mc.getTextureManager()
                        .bindTexture(mc.thePlayer.getLocationSkin());
            ...
            if (!VoiceChatClient.getSoundManager().currentStreams.isEmpty() && voiceChat.getSettings()
                ...
                for (int i = 0; i < VoiceChatClient.getSoundManager().currentStreams.size(); i++) {
                    ...
                    if (stream != null) {
                        ...
                        glPushMatrix();
                        glTranslatef(
                            position.x + positionUI.info.offsetX,
                            position.y + positionUI.info.offsetY + ((i * 23) * scale),
                            0);
