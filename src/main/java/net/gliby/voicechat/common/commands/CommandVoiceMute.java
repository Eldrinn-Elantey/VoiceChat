...
public class CommandVoiceMute extends CommandBase {
    ...
    public void processCommand(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
        if (par2ArrayOfStr.length == 1 && par2ArrayOfStr[0].length() > 0) {
            final ServerNetwork network = VoiceChat.getServerInstance()
                .getServerNetwork();
            ...
            if (player != null) {
                if (network.getDataManager().mutedPlayers.contains(player.getUniqueID())) {
                    ...
                    func_152373_a(
                        par1ICommandSender,
                        this,
                        player.getDisplayName() + " has been unmuted.",
                    ...
                    func_152373_a(
                        par1ICommandSender,
                        this,
                        player.getDisplayName() + " has been muted.",
                        new Object[] { par2ArrayOfStr[0] });
                    network.getDataManager().mutedPlayers.add(player.getUniqueID());
                    player.addChatMessage(
                        new ChatComponentText(
                            "You have been voice muted, you cannot talk untill you have been unmuted."));
                }
