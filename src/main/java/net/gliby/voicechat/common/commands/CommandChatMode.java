...
public class CommandChatMode extends CommandBase {
    ...
    public List addTabCompletionOptions(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
        return par2ArrayOfStr.length == 1
            ? getListOfStringsMatchingLastWord(par2ArrayOfStr, new String[] { "distance", "global", "world" })
    ...
    protected int getChatModeFromCommand(ICommandSender par1ICommandSender, String par2Str) {
        return (par2Str.equalsIgnoreCase("distance") || par2Str.startsWith("d") || par2Str.equalsIgnoreCase("0")) ? 0
            : ((par2Str.equalsIgnoreCase("world") || par2Str.startsWith("w") || par2Str.equalsIgnoreCase("1")) ? 1
                : (par2Str.equalsIgnoreCase("global") || par2Str.startsWith("g") || par2Str.equalsIgnoreCase("2")) ? 2
                    : 0);
    }
    ...
    public void processCommand(ICommandSender par1ICommandSender, String[] par2ArrayOfStr) {
        ...
        if (par2ArrayOfStr.length > 0) {
            ...
            final EntityPlayerMP player = par2ArrayOfStr.length >= 2 ? getPlayer(par1ICommandSender, par2ArrayOfStr[1])
                : getCommandSenderAsPlayer(par1ICommandSender);
            if (player != null) {
                final ServerStreamManager dataManager = VoiceChat.getServerInstance()
                    .getServerNetwork()
                    .getDataManager();
                ...
                if (player != par1ICommandSender) {
                    func_152373_a(
                        par1ICommandSender,
                        this,
                        player.getCommandSenderName() + " set chat mode to "
                            + getChatMode(chatMode).toUpperCase()
                            + " ("
                            + chatMode
                            + ")",
                    ...
                    player.addChatMessage(
                        new ChatComponentText(
                            "Set own chat mode to " + getChatMode(chatMode).toUpperCase() + " (" + chatMode + ")"));
                    switch (chatMode) {
                        case 0:
