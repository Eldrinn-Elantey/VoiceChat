...
public class ServerNetwork {
    ...
    public EntityPlayerMP[] getPlayers() {
        final List<EntityPlayerMP> pl = MinecraftServer.getServer()
            .getConfigurationManager().playerEntityList;
    ...
    public void init() {
        if (voiceChat.getServerSettings()
            .isUsingProxy()) {
    ...
    private String retrieveExternalAddress() {
        VoiceChat.getLogger()
            .info("Retrieving server address.");
    ...
    public void sendEntityData(EntityPlayerMP player, int entityID, String username, double x, double y, double z) {
        VoiceChat.getDispatcher()
            .sendTo(new MinecraftClientEntityDataPacket(entityID, username, x, y, z), player);
    }
