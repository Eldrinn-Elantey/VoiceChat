...
public class ServerStreamManager {
    ...
    public void createStream(ServerDatalet data) {
        ...
        VoiceChatAPI.instance()
            .bus()
            .post(new ServerStreamEvent.StreamCreated(this, stream, data));
        ...
        if (voiceData.end) {
            for (int i = 0; i < players.size(); i++) {
                ...
                if (target.getEntityId() != speaker.getEntityId()) {
                    voiceChat.getVoiceServer()
                        .sendVoiceEnd(target, voiceData.id);
                }
            }
        } else {
            for (int i = 0; i < players.size(); i++) {
     ...
     */
    public void feedStreamToPlayer(ServerStream stream, ServerDatalet voiceData, EntityPlayerMP target,
        boolean direct) {
        final EntityPlayerMP speaker = voiceData.player;
        if (voiceData.end) if (voiceChat.getVoiceServer() != null && target != null) voiceChat.getVoiceServer()
            .sendVoiceEnd(target, stream.id);
        else {
            entityHandler.whileSpeaking(stream, speaker, target);
            voiceChat.getVoiceServer()
                .sendChunkVoiceData(target, voiceData.id, direct, voiceData.data, voiceData.divider, voiceData.volume);
        }
    }
    ...
    public void feedStreamToWorld(ServerStream stream, ServerDatalet voiceData) {
        ...
        if (voiceData.end) {
            for (int i = 0; i < players.size(); i++) {
                ...
                final EntityPlayerMP target = (EntityPlayerMP) player;
                if (target.getEntityId() != speaker.getEntityId()) {
                    if (voiceChat.getVoiceServer() != null && target != null) voiceChat.getVoiceServer()
                        .sendVoiceEnd(target, stream.id);
                }
            }
        } else {
                ...
                if (target.getEntityId() != speaker.getEntityId()) {
                    ...
                    if (distanceBetween < distance * distance) {
                        ...
                        if (stream.tick % voiceChat.serverSettings.positionUpdateRate == 0) {
                            if (distanceBetween > 64 * 64) voiceChat.getVoiceServer()
                                .sendEntityPosition(
                                    target,
                                    speaker.getEntityId(),
                                    speaker.posX,
    ...
    public void giveEntity(EntityPlayerMP receiver, EntityPlayerMP speaker) {
        voiceChat.getServerNetwork()
            .sendEntityData(
                receiver,
                speaker.getEntityId(),
                speaker.getCommandSenderName(),
                speaker.posX,
                speaker.posY,
                speaker.posZ);
    ...
    public void giveStream(ServerStream stream, ServerDatalet let) {
        VoiceChatAPI.instance()
            .bus()
            .post(new ServerStreamEvent.StreamFeed(this, stream, let));
    ...
    public void killStream(ServerStream stream) {
        ...
        VoiceChatAPI.instance()
            .bus()
            .post(new ServerStreamEvent.StreamDestroyed(this, stream));
    }
