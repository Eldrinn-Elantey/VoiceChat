    ...
    protected void runServer() {
        try {
            ...
            try {
                ...
                LOGGER.warn(
                    String.format(
                        "Could not set receive buffer to %d. It is now at %d. Error: %s",
                        pl,
                        bl,
                        exc.getMessage()));
            } // end catch
            ...
            synchronized (This) {
                if (this.currentState == State.STOPPING) { // User asked to stop
                    ...
                    LOGGER.warn(
                        "If the server cannot bind: Switch to Minecraft Networking in config or setup UDP properly, that means port-forwarding.");
