...
public class MinecraftClientEntityPositionPacket extends MinecraftPacket
    implements IMessageHandler<MinecraftClientEntityPositionPacket, IMessage> {
    ...
    public IMessage onMessage(MinecraftClientEntityPositionPacket packet, MessageContext ctx) {
        if (VoiceChat.getProxyInstance()
            ...
            VoiceChat.getProxyInstance()
                .getClientNetwork()
                .getVoiceClient()
                .handleEntityPosition(packet.entityID, packet.x, packet.y, packet.z);
        return null;
    }
