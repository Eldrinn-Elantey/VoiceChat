...
public abstract class VoiceServer implements Runnable {
    ...
    public final void run() {
        VoiceChat.getLogger()
            .info(
                this.start() ? "Started [" + getType().name + "] Server."
                    : "Failed to start [" + getType().name + "] Server.");
    }

    public abstract void sendChunkVoiceData(EntityPlayerMP player, int entityID, boolean direct, byte[] samples,
        byte chunkSize, byte volume);
