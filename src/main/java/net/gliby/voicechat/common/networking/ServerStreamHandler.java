...
public class ServerStreamHandler {
    ...
    public void createdStream(ServerStreamEvent.StreamCreated event) {
        ...
        int chatMode = voiceChat.getServerSettings()
            .getDefaultChatMode();
    ...
    public void feedStream(ServerStreamEvent.StreamFeed event) {
        ...
        if (event.stream.dirty) {
            if (event.streamManager.chatModeMap.containsKey(event.stream.player.getPersistentID()))
        ...
        switch (event.stream.chatMode) {
            case 0:
                event.streamManager.feedWithinEntityWithRadius(
                    event.stream,
                    event.voiceLet,
