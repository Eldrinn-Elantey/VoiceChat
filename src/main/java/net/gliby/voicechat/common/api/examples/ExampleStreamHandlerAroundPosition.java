...
public class ExampleStreamHandlerAroundPosition {
    ...
    public void feedStream(ServerStreamEvent.StreamFeed event) {
        ...
        feedStreamPositionWithRadius(
            event.streamManager,
            event.stream,
            event.voiceLet,
            event.stream.player.worldObj,
            0,
            128,
            0,
            VoiceChat.getServerInstance()
                .getServerSettings()
                .getSoundDistance());
    }
     ...
     * @param y
     * @param z
     * @param distance      Distance should be the same as the distance in voice chat settings, otherwise it will stream
     *                      globally.
     */
    public void feedStreamPositionWithRadius(ServerStreamManager streamManager, ServerStream stream,
        ServerDatalet voiceData, World world, double x, double y, double z, int distance) {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            if (target.getEntityId() != speaker.getEntityId()) {
                ...
                if (d4 * d4 + d5 * d5 + d6 * d6 < distance * distance) {
                    streamManager.feedStreamToPlayer(
                        stream,
                        voiceData,
                        target,
                        distance == VoiceChat.getServerInstance()
                            .getServerSettings()
                            .getSoundDistance());
                }
            }
