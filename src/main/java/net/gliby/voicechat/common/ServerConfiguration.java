...
public class ServerConfiguration {
    ...
    private static final String MODPACK_ID = "ModPackID", BEHIND_PROXY = "ServerBehindProxy",
        SHOW_VOICEPLATES = "ShowVoicePlates", SHOW_PLAYERICONS = "ShowPlayerIcons", MINIMUM_QUALITY = "MinimumQuality",
        MAXIMUM_QUALITY = "MaximumQuality", SOUND_DISTANCE = "SoundDistance", DEFAULT_CHAT_MODE = "DefaultChatMode",
        UDP_PORT = "UDPPort", NETWORK_TYPE = "NetworkType", BUFFER_SIZE = "BufferSize";
    ...
    public void init() {
        if (!load()) {
            VoiceChat.getLogger()
                .info("No Configuration file found on server, will create one with default settings.");
            if (save()) VoiceChat.getLogger()
                .info("Created Configuration file with default settings on server.");
        }
    }
    ...
    private boolean load() {
        if (location.exists()) {
            try {
                ...
                settings.setSoundDistance(
                    init.ReadFloat("Game", SOUND_DISTANCE, Float.valueOf(64))
                        .intValue());
                ...
                settings.setUDPPort(
                    init.ReadInteger(
                        "Network",
                        UDP_PORT,
                        Integer.valueOf(settings.getUDPPort())
                            .intValue()));
                settings.setQuality(
                    init.ReadInteger(
                        "Network",
                        MINIMUM_QUALITY,
                        Integer.valueOf(settings.getMinimumSoundQuality())
                            .intValue()),
                    init.ReadInteger(
                        "Network",
                        MAXIMUM_QUALITY,
                        Integer.valueOf(settings.getMaximumSoundQuality())
                            .intValue()));
                settings.setBufferSize(
                    init.ReadInteger(
                        "Network",
                        BUFFER_SIZE,
                        Integer.valueOf(settings.getBufferSize())
                            .intValue()));
                ...
                VoiceChat.getLogger()
                    .fatal("Couldn't read configuration file, fix it or delete it. Default settings being used.");
                e.printStackTrace();
            }
        }
        return false;
    ...
    public boolean save() {
        ...
        this.init.WriteComment(
            "Game",
            SHOW_PLAYERICONS + ", if false - players won't see icons when someone talks; "
                + SHOW_VOICEPLATES
                + ", if false - players won't see player names(voice plates) on their screens.");
        this.init.WriteInteger("Network", NETWORK_TYPE, settings.getAdvancedNetworkType());
        this.init.WriteComment(
            "Network",
            NETWORK_TYPE
                + ", 0 - Minecraft Network, 1 - UDP Network. UDP networking improves performance and network speeds extensively, it is highly recommended.");
        this.init.WriteInteger("Network", UDP_PORT, settings.getUDPPort());
        this.init.WriteComment(
            "Network",
            "If " + UDP_PORT
                + " is set to 0, minecraft's own port will be used, this cannot be the same as query port! Change the network type to 0, if you can't port forward to a UDP custom port.");
        ...
        this.init
            .WriteComment("Network", BEHIND_PROXY + ": if server is behind a proxy, like bungeecord, enable this.");
